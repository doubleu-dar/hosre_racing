import type { MapBuilder } from "./types.ts";
import {
  addPins,
  addSpinningRect,
  addSpinningCross,
...
// DropBallScene의 makeWalls 함수 로직을 맵 생성에 포함
function makeWalls(ctx: any) {
...
// 기본 맵 생성 함수 (DropBallScene의 장애물 배치 참고)
const defaultMap: MapBuilder = (ctx: any) => {
  ...
  addSpinningCross(ctx, ctx.leftX + (ctx.width / 4) * 1, 1000, 250, 20, -0.03);
  addSpinningCross(ctx, ctx.leftX + (ctx.width / 4) * 3, 1000, 250, 20, 0.03);

  makeFunnels(ctx);
  addPins(ctx, centerX, 7, 3, 1500);
  addSpinningRect(ctx, ctx.rightX + (ctx.width / 3) * 1, 3800, 500, 20, 0.005);
  addSpinningRect(ctx, ctx.rightX + (ctx.width / 3) * 2, 4200, 500, 20, -0.005);
  addSpinningRect(ctx, ctx.leftX + (ctx.width / 4) * 1, 5300, 400, 20, -0.01);

  addTriangles(ctx, centerX, 5, 5, 5500);
  makeLastFunnel(ctx);

  // 핀(peg) 배열 생성 (중앙 기준, 8행)
  // addPins(ctx, (ctx.leftX + ctx.rightX) / 2, 7, 8, 400);

  // // 회전 사각바(스피닝 바) - 중간/아래 구간에 2개
  // addSpinningRect(ctx, (ctx.leftX + ctx.rightX) / 2, 1200, 400, 20, 0.01);
  // addSpinningRect(ctx, (ctx.leftX + ctx.rightX) / 2, 2200, 400, 20, -0.012);

  // // 십자 회전 장애물 (cross) - 상단/중단
  // addSpinningCross(ctx, (ctx.leftX + ctx.rightX) / 2, 700, 200, 20, 0.03);
  // addSpinningCross(ctx, (ctx.leftX + ctx.rightX) / 2, 1800, 200, 20, -0.025);

  // 회전 삼각형 장애물 - 하단 구간
  // addTriangles(ctx, (ctx.leftX + ctx.rightX) / 2, 5, 2, 3000, 0.04);
};

function makeFunnels(ctx: any) {
...
function makeLastFunnel(ctx: any) {
